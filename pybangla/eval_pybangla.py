...
def evaluate_normalization(data, output_path):
    ...
    for idx, (input_text, reviewed_text, category) in enumerate(
        tqdm(
            zip(input_texts, reviewed_texts, categories), total=len(input_texts), desc="Processing"
        )
    ):
        # Normalize text
        ...
        if not exact_match:
            ...
            differences = "; ".join(
                [
                    f"{d['operation']}: '{d['reviewed']}' → '{d['normalized']}' at {d['position']}"
                    for d in diff_details[:3]
                ]
    ...
    error_df = error_df[
        ["Index", "Input_Text", "Human_Review", "Normalized_Text", "CER", "Differences"]
    ]
...
if __name__ == "__main__":
    ...
    output_eval_data_path = "./report/eval_data_3.0.0.xlsx"
